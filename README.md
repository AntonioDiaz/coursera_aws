...
- [Week 05](#week-05)
    - [Reading 01: X-Ray Terminology](#reading-01-x-ray-terminology)
        - [AWS X-Ray Terminology](#aws-x-ray-terminology)
        - [The X-Ray Daemon](#the-x-ray-daemon)
        - [How to use AWS X-Ray in Java Programs](#how-to-use-aws-x-ray-in-java-programs)
        - [Using Aspect-Oriented-Programming and AWS X-Ray](#using-aspect-oriented-programming-and-aws-x-ray)
...
* Annotations and metadata
    ...
    * __Annotations__ are simple key-value pairs that are indexed for use with filter expressions. Use annotations to record data that you want to use to group traces in the console, or when calling the GetTraceSummaries API.

    * X-Ray indexes up to 50 annotations per trace.

    * __Metadata__ are key-value pairs with values of any type, including objects and lists, but that are not indexed. Use metadata to record data you want to store in the trace but don't need to use for searching traces.
...
#### How to use AWS X-Ray in Java Programs
* The X-Ray SDK for Java is a set of libraries for Java web applications that provide classes and methods for generating and sending trace data to the X-Ray daemon.  
...
For more information on the AWS X-Ray SDK for Java click here: https://docs.aws.amazon.com/xray/latest/devguide/xray-sdk-java.html


#### Using Aspect-Oriented-Programming and AWS X-Ray
If you use the Spring framework and want to use AWS X-ray as an aspect click here for more information: https://docs.aws.amazon.com/xray/latest/devguide/xray-sdk-java-aop-spring.html
...
```
* Create Subsegment (Lambda will create a segment for us):
```java
    private static String queryS3(SelectObjectContentRequest request) {
...
<img width="1642" alt="cloudwatch_logs" src="https://user-images.githubusercontent.com/725743/130580816-2f6b2c07-1db5-48c3-8e3e-1b257cc109a9.png">
...
Configuring Lambda to send logs to CloudWatch Logs isn’t a difficult task. In fact, the only step is to make sure that Lambda is allowed to create a Log Group, create a Log Stream and put Log Events. This can easily be done by using the IAM managed policy “AWSLambdaBasicExecutionRole”. Then, it’s only a matter of using the java.lang.System library or any logging module that writes to stdout or stderr in your code. To make things even simpler, the aws-lambda-java-core library has a logger class called LambdaLogger which can be accessed using the context variable that is passed to the Lambda handler. You can find more information on how to use it here:  
